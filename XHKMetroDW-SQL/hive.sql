...
deal_money String,
equ_no String)
PARTITIONED BY(DAY STRING) ROW FORMAT DELIMITED FIELDS TERMINATED BY ',' LOCATION '/user/warehouse/szt.db/ods/ods_szt_data';

LOAD DATA INPATH '/user/warehouse/szt.db/ods/szt-data_2018-09-01.csv' OVERWRITE INTO TABLE ods_szt_data PARTITION(DAY = '2018-09-01');

select * from ods_szt_data
where day = '2018-09-01'
limit 10;

select collect_set(deal_type) from ods_szt_data;

select count(1) from ods_szt_data;   --217037
...
deal_money String,
equ_no String)
PARTITIONED BY(DAY STRING) ROW FORMAT DELIMITED FIELDS TERMINATED BY ',' LOCATION '/user/warehouse/szt.db/dwd/dwd_fact_szt_in_out_detail';
...
SELECT count(1) FROM dwd_fact_szt_in_out_detail; --210269
...
deal_money String,
equ_no String)
PARTITIONED BY(DAY STRING) ROW FORMAT DELIMITED FIELDS TERMINATED BY ',' LOCATION '/user/warehouse/szt.db/dwd/dwd_fact_szt_in_detail';
...
select count(1) from dwd_fact_szt_in_detail;  --136433

select * from dwd_fact_szt_in_detail
limit 10;
...
deal_money String,
equ_no String)
PARTITIONED BY(DAY STRING) ROW FORMAT DELIMITED FIELDS TERMINATED BY ',' LOCATION '/user/warehouse/szt.db/dwd/dwd_fact_szt_out_detail';
...
select count(1) from dwd_fact_szt_out_detail;  --73836
...
`count` int)
PARTITIONED BY(DAY STRING) ROW FORMAT DELIMITED FIELDS TERMINATED BY ',' LOCATION '/user/warehouse/szt.db/dws/dws_card_record_day_wide';
...
equ_no_arr ARRAY < STRING > ,
`count` int)
PARTITIONED BY(DAY string) ROW FORMAT DELIMITED FIELDS TERMINATED BY ',' LOCATION '/user/warehouse/szt.db/ads/ads_in_station_day_top';
...
select count(1) from ads_in_station_day_top;
...
equ_no_arr ARRAY < STRING > ,
`count` bigint)
PARTITIONED BY(DAY string) ROW FORMAT DELIMITED FIELDS TERMINATED BY ',' LOCATION '/user/warehouse/szt.db/ads/ads_out_station_day_top';
...
equ_no_arr ARRAY < STRING > ,
`count` int)
PARTITIONED BY(DAY string) ROW FORMAT DELIMITED FIELDS TERMINATED BY ',' LOCATION '/user/warehouse/szt.db/ads/ads_in_out_station_day_top';
...
equ_no_arr ARRAY<STRING>,
`count` INT)
PARTITIONED BY(DAY string) ROW FORMAT DELIMITED FIELDS TERMINATED BY ',' LOCATION '/user/warehouse/szt.db/ads/ads_card_deal_day_top';
...
from
    ...
    (SELECT card_no,
            sum(deal_v) OVER(PARTITION BY card_no) AS deal_sum
     FROM dws_card_record_day_wide
     LATERAL VIEW explode(deal_value_arr) tmp as deal_v )t2,
    (SELECT card_no,
            sum(deal_m) OVER(PARTITION BY card_no) AS deal_m_sum
     FROM dws_card_record_day_wide
     LATERAL VIEW explode(deal_money_arr) tmp as deal_m )t3
...
FROM dws_card_record_day_wide
LATERAL VIEW explode(deal_value_arr) tmp as deal_v
ORDER BY deal_sum DESC;--ok
...
PARTITIONED BY(DAY STRING) ROW FORMAT DELIMITED FIELDS TERMINATED BY ',' LOCATION '/user/warehouse/szt.db/ads/ads_line_send_passengers_day_top';
...
partitioned by(day string) row format delimited fields terminated by ',' location '/user/warehouse/szt.db/dws/dws_in_out_sorted_card_date';
...
deal_date,
deal_type_station,
LEAD(deal_type_station,1) over(partition by card_no order by deal_date) as next_station  --lead()
from temp02;
...
from temp03
where substr(deal_type_station,0,4)='地铁入站' and substr(next_station,0,4)='地铁出站';
...
partitioned by(day string) row format delimited fields terminated by ',' location '/user/warehouse/szt.db/ads/ads_stations_send_passengers_day_top';
...
select * from ads_stations_send_passengers_day_top;
----------------------------------------------------------------------------------
...
partitioned by(day string) row format delimited fields terminated by ',' location '/user/warehouse/szt.db/dws/dws_in_out_sorted_card_date_wide';
...
row format delimited fields terminated by ',' location '/user/warehouse/szt.db/temp/temp02'
as select
card_no,
ts,
deal_type,
...
row format delimited fields terminated by ',' location '/user/warehouse/szt.db/temp/temp033'
as select
card_no,
ts,
company_name,
...
row format delimited fields terminated by ',' location '/user/warehouse/szt.db/temp/temp044'
as select
card_no,
company_name,
conn_mark,
...
partitioned by(day string) row format delimited fields terminated by ',' location '/user/warehouse/szt.db/dws/dws_travel_info';
...
row format delimited fields terminated by ',' location '/user/warehouse/szt.db/temp/temp055'
as select
card_no,
company_name,
...
partitioned by(day string) row format delimited fields terminated by ',' location '/user/warehouse/szt.db/ads/ads_conn_spend_time_top';
...
load data inpath '/user/warehouse/szt.db/ods/szmc.net-metro.csv' overwrite into table ods_line_station;
...
(
    ...
    row format delimited fields terminated by '\t' location '/user/warehouse/szt.db/ods/ods_travel_plan';

load data inpath '/user/warehouse/szt.db/ods/travel_plan.csv' overwrite into table ods_travel_plan;
...
drop table if exists dwd_travel_plan_detail;
create table dwd_travel_plan_detail row format delimited fields terminated by '\t' location '/user/warehouse/szt.db/dwd/dwd_travel_plan_detail' as
select start_station_id,
